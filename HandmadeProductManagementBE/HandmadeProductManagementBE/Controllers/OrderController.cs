...
namespace HandmadeProductManagementAPI.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        [HttpGet("seller")]
        public async Task<IActionResult> GetOrderForSeller([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10)
        {
            var userId = User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier)?.Value ?? string.Empty;
            var orders = await _orderService.GetOrdersBySellerUserIdAsync(Guid.Parse(userId), pageNumber, pageSize);
